...
//   // Dummy auth state to trigger the Avatar UI (will be replaced by Redux later)
...
//                     </div>
//                   ) : (
//                     <Button onClick={() => navigate("/login")} className="text-sm font-medium text-gray-700 hover:text-gray-800">
//                       Sign in
//                     </Button>
//                   )}
//                 </div>
...
export default function Navigation() {
  ...
  const handleClose = () => {
    ...
    // navigate("/"); // Go back to previous page after closing modal
  };
  ...
  // Close modal automatically when route changes (e.g., successful login/register)
  useEffect(() => {
    if (auth.user) {
      handleClose();
    }
        ...
        <nav aria-label="Top" className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="border-b border-gray-200">
            <div className="flex h-16 items-center">
              ...
              {/* Flyout menus (Keep your existing Desktop menu Popover code here) */}
              <Popover.Group className="hidden lg:ml-8 lg:block lg:self-stretch z-10">
                <div className="flex h-full space-x-8">
                  {navigation.categories.map((category) => (
                    <Popover key={category.name} className="flex">
              ...
              <div className="ml-auto flex items-center">
                <div className="hidden lg:flex lg:flex-1 lg:items-center lg:justify-end lg:space-x-6">
                  ...
                  {/* Conditional Rendering for Auth: Show Avatar if logged in, else show Sign In button */}
                  {auth.user?.firstName ? (
                    <div>
                      <Avatar
